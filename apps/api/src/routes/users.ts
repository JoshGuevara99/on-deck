...
const UpdateUserSchema = z.object({
  ...
  tiktokHandle: z.string().max(60).nullable().optional(),
  ...
  try {
    ...
    const user = await prisma.user.update({
      ...
      data: {
        ...
        ...(input.instagramHandle !== undefined && { instagramHandle: input.instagramHandle?.trim().replace(/^@/, '') || null }),
        ...(input.tiktokHandle !== undefined && { tiktokHandle: input.tiktokHandle?.trim().replace(/^@/, '') || null }),
